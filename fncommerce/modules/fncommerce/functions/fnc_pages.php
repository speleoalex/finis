...
function print_saveorder()
{
    ...
    if ($_FN['user'] != '')
    {
        $table = FN_XMDBForm("fnc_conditions"); //new FieldFrm("fndatabase","fnc_conditions",$_FN['datadir']);
        $all = $table->xmltable->GetRecords(false, false, false, "sort_order");
        //echo "<br /><br /><form method=\"post\" action=\"?mod={$_FN['mod']}&amp;op=saveorder\">";
        if (is_array($all) && count($all) > 0)
        {
            foreach ($all as $item)
            {
                if (!isset($_POST['conditions' . $item['unirecid']]))
                {
                    ...
                    echo FN_Translate("you must accept the terms of purchase") . "<br /><br /><a href=\"?{$_FN['mod']}&amp;op=confirmorder\">" . FN_translate("back") . "</a>";
                    return;
                }
            }
        }
        $orderstatus = fnc_get_order_temp();
        //dprint_r($orderstatus);
        //echo html_orderstatus($newstate['unirecid']);
        //die();
        ...
        if (isset($orderstatus['cart']))
        {
            ...
            //elimina ordine temporaneo ---<
            echo "<div>" . (FN_Translate("your order has been forwarded")) . "</div>";

            echo html_orderstatus($newstate['unirecid']);
            echo "<div><b>" . FN_Translate("thanks for your order") . "</b></div>";
        }
        echo "<br /><button onclick=\"window.location='?mod={$_FN['mod']}&op=orderstatus'\">" . FN_Translate("view my orders") . "</button>";
    }
...
function print_confirm()
{
    ...
    $cart = fnc_get_cart();
    echo "<form method=\"post\" action=\"?mod={$_FN['mod']}&amp;op=saveorder\">";
    echo "<input type=\"submit\" value=\"" . FN_Translate("confirm this order") . "  &gt;&gt;\" /><br />";
    echo "<table width=\"100%\" border=\"0\">";
    echo "<tr>";
    echo "<td valign=\"top\" >";
    echo "<b>" . FN_Translate("my orders") . ":</b>";
    $orderstatus = fnc_calculate_order_cost($orderstatus);
    echo fnc_get_ordercost_details($orderstatus);
    echo "&nbsp;<a href=\"?mod={$_FN['mod']}&amp;op=showcart\">[" . FN_i18n("modify") . "]</a>";
    $orderstatus = fnc_save_order_temp($orderstatus);
    echo "</td>";
    echo "<td valign=\"top\">";

    //-------riassunto steps scelti ------>
    ...
    foreach ($steps as $step)
    {
        if (isset($orderstatus[$step]))
        {
            ...
            $payment = new $classname($orderstatus);
            echo $payment->title();
            ...
            echo "<br /><a href=\"?mod={$_FN['mod']}&amp;op=ordersteps&amp;orderstep=$step\">[" . FN_i18n("modify") . "]</a><br /><br />";
        }
    }
    //-------riassunto steps scelti ------<
    ...
    echo "</td>";
    echo "<td valign=\"top\">";
    echo "<b>" . FN_Translate("customer data") . ":</b><br />";
    $shippingvalues = fnc_get_shipping_values($orderstatus['username']);
    $shippingvalues = $shippingvalues[0];
    print_shipping_values($shippingvalues);
    echo "<a href=\"" . FN_RewriteLink("?mod={$_FN['mod']}&amp;op=shipping") . "\">[" . FN_i18n("modify") . "]</a>";
    echo "</td>";
    //echo "<td valign=\"top\"><input type=\"submit\" value=\"" . FN_Translate("confirm this order") . " &gt;&gt;\" /></td>";
    //dprint_r($orderstatus);
    //------condizioni di vendita------->
    $table = FN_XMDBForm("fnc_conditions"); // new FieldFrm("fndatabase","fnc_conditions",$_FN['datadir'],$_FN['lang']);
    $all = $table->xmltable->GetRecords(false, false, false, "sort_order");
    if (is_array($all) && count($all) > 0)
    {
        ...
        foreach ($all as $item)
        {
            ...
            echo "<br /><b>" . $item['title'] . ":</b>&nbsp;&nbsp;<input style=\"vertical-align:middle\" name=\"conditions{$item['unirecid']}\" type=\"checkbox\" />" . _ACCEPT . "<br /><div style=\"border:1px inset;height:100px;overflow:auto\" >{$item['text']}</div>";
            //echo "<br /><input name=\"conditions{$item['unirecid']}\" type=\"checkbox\" />"._ACCEPT."<br />";
        }
        echo "</td></tr>";
    }
    //------condizioni di vendita-------<
    ...
    echo "<tr><td colspan=\"3\" style=\"text-align:left\">";
    echo "<br /><input type=\"submit\" value=\"" . FN_Translate("confirm this order") . " &gt;&gt;\" />";
    echo "</td></tr>";
    echo "</table>";
    echo "</form>";
}
...
function print_current_order_step($step)
{
    ...
    if (isset($old[$step]))
        $old = $old[$step];
    echo "<form method=\"post\" action=\"?mod={$_FN['mod']}&amp;orderstep=$step&amp;op=$op\" >";

    foreach ($list_modules_in_this_step as $payment)
    {
        //---------------visializzazione opzione ----------------->
        $sid = $payment;
        if ($sid != "")
        {
            ...
            $stepclass = new $classname($orderstatus);
            $stepclass->show_option($orderstatus);
        }
        //---------------visializzazione opzione -----------------<
    }
    echo "<br /><br />";
    ...
    if ($pevstep == "")
    {
        $link = fn_rewritelink("?mod={$_FN['mod']}&op=shipping");
    }
    else
    {
        $link = fn_rewritelink("?mod={$_FN['mod']}&op=$op&orderstep=$pevstep");
    }
    //echo "p=$pevstep,c=$step";
    echo "<input type=\"button\" onclick=\"window.location='$link'\" value=\"" . FN_i18n("back") . "\" />";
    //echo "<input type=\"button\" onclick=\"history.back()\" value=\"" . FN_i18n("back") . "\" />";
    echo "&nbsp;<input type=\"submit\" value=\"" . FN_i18n("next") . " &gt;&gt;\" />";
    echo "</form>";
}
...
function print_shipping()
{
    ...
    if (!fnc_get_cart())
    {
        echo FN_Translate("your cart is empty") . "<br /><br />";
        echo "&nbsp;&nbsp;<a href=\"" . fn_rewritelink("?mod={$_FN['mod']}") . "\">" . FN_translate("continue shopping") . "</a>";
        return;
    }
    ...
    if ($_FN['fnc_enable_recipient'] == 0 || order_is_freeeshipping($order))
    {
        ...
        //dprint_r($Table->xmltable->fields);
        //	echo "adasd";
    }
    ...
    else
    {
        ...
        $Table->setlayoutTags("<div style=\"float:left;width:150px;\"><nobr>", ":</nobr></div>", "", "<br />", "<fieldset><legend>", "</legend>", "</fieldset>" . "<br />");
        ...
        echo "<b>" . FN_Translate("please complete the shipping information") . "</b><br />";
        echo "<em>" . FN_i18n("required fields") . "</em><hr />";
        if ($edit != "")
        {
            echo "<form method=\"post\" action=\"?mod={$_FN['mod']}&amp;op=$op&amp;update=1&amp;edit=$edit\">";
            echo "<input type=\"hidden\" name=\"unirecid\" value=\"$edit\" />";
            $values = $Table->xmltable->GetRecordByPrimaryKey($edit);
            if ($_FN['user'] != $values['username'] && !FN_IsAdmin())
                die(FN_i18n("operation not allowed"));
            $all = $Table->ShowUpdateForm($edit, FN_IsAdmin(), $newvalues);
            echo "<fieldset><legend>" . FN_Translate("notes") . "</legend><textarea style=\"width:100%\" rows=\"3\" name=\"notes\">" . htmlspecialchars($notes) . "</textarea></fieldset><br />";
            echo "<span style=\"color:red\">$errortext</span><br />";
            echo "<input onclick=\"window.location='?mod={$_FN['mod']}&op=showcart'\" type=\"button\" value=\"" . FN_i18n("back") . "\"/>";
            echo "&nbsp;<input name=\"save\" type=\"submit\" value=\"" . FN_i18n("next") . " &gt;&gt;\"/>";
            echo "</form>";
        }
        else
        {
            echo "<form method=\"post\" action=\"?mod={$_FN['mod']}&amp;op=$op\">";
            $all = $Table->ShowInsertForm(FN_IsAdmin(), $newvalues);
            echo "<fieldset><legend>" . FN_Translate("notes") . "</legend><textarea style=\"width:100%\" rows=\"3\" name=\"notes\">" . htmlspecialchars($notes) . "</textarea></fieldset><br />";
            echo "<span style=\"color:red\">$errortext</span><br />";
            echo "<input onclick=\"window.location='?mod={$_FN['mod']}&op=showcart'\" type=\"button\" value=\"" . FN_i18n("back") . "\"/>";
            echo "<input name=\"save\" type=\"submit\" value=\"" . FN_i18n("next") . " &gt;&gt;\"/>";
