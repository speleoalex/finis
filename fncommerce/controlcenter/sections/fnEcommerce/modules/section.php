...
if ($edit == "") {
	...
	foreach ($liststeps as $itemstep) {
		...
		echo "</td>";
		$title = FN_GetFolderTitle("{$fncommerce_path}/modules/$itemstep");
		...
		if ($list_enabled_modules) {
			foreach ($listitems_tmp as $itemdir) {
				...
				if ($itemdir && !in_array($itemdir, $listitems)) {
					$listitems[] = $itemdir;
				}
			}
		}
		...
		foreach ($listitems as $item) {
			if ($item) {
				...
				$title = FN_GetFolderTitle("{$fncommerce_path}/modules/$itemstep/$item");
				echo "<td>{$title}</td>";
				echo "<td>";
